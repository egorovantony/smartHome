...
public class Sensor {
    ...
    public Sensor(ResultSet rs) throws MBClientModelException{
        String strType;
        String strTypeWrite;
        ...
        try {
            ...
            throw new MBClientModelException("Ошибка при чтении данных датчика.");
        }

        if (    this.ID == 0 ||
                this.IDContr == 0 ||
                this.ContrOrder == 0){
            throw new MBClientModelException("Не все поля заполнены корректно для датчика с ID=" + this.ID);
        }

        try{
            this.type = TypeSens.valueOf(strType);
        }catch(IllegalArgumentException ex){
            throw new MBClientModelException("Некорректное значение поля Type для датчика с ID=" + this.ID);
        }

        try{
            this.typeWrite = TypeWriteSens.valueOf(strTypeWrite);
        }catch(IllegalArgumentException ex){
            throw new MBClientModelException("Некорректное значение поля TypeWrite для датчика с ID=" + this.ID);
        }
